...
#define MAP_WIDTH 700		/* default map width */
...
static GtkWidget *splash_build_page(void)
{
	...
	filename = g_build_filename(g_get_user_runtime_dir(), "pixmaps", "pioneers",
				    "splash.png", NULL);
...
static void register_pixmaps(void)
{
	...
	for (idx = 0; idx < G_N_ELEMENTS(pioneers_pixmaps); idx++) {
		...
		filename = g_build_filename(g_get_user_runtime_dir(), "pixmaps",
